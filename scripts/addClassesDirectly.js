...
// Hàm thêm lớp học trực tiếp vào database
const addClassesDirectly = async () => {
  try {
    console.log('Starting to add classes for courses...');
    
    // Lấy tất cả khóa học trực tiếp từ database
    ...
    for (const course of courses) {
      ...
      for (let i = 0; i < 3; i++) {
        ...
        await db.collection('classes').insertOne(newClassData);
